...
const char * const ODUWAUSD_COIN_CONF_FILENAME = "oduwausd_coin.conf";
const char * const ODUWAUSD_COIN_PID_FILENAME = "oduwausd_coind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "oduwausd_coind -foo=bar
...
#else
    const char* pszModule = "oduwausd_coin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\OduwaUSD_Coin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\OduwaUSD_Coin
    // Mac: ~/Library/Application Support/OduwaUSD_Coin
    // Unix: ~/.oduwausd_coin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "OduwaUSD_Coin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/OduwaUSD_Coin";
#else
    // Unix
    return pathRet / ".oduwausd_coin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", ODUWAUSD_COIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", ODUWAUSD_COIN_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure OduwaUSD Coin copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("OduwaUSD Coin") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The OduwaUSD Coin developers";
    }
