...
#if defined(NDEBUG)
# error "OduwaUSD_Coin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "OduwaUSD_Coin Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s OWDT additional fees, %d delta bytes\n",
...
void ThreadScriptCheck() {
    RenameThread("oduwausd_coin-scriptch");
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
	...
	if (fCheckPOW && block.GetHash() != consensusParams.hashGenesisBlock) {
		CTxDestination destination = DecodeDestination("ocKYVCJDyFXpdZAeR2xPCvopkrMU7RCE6W");
		const CKeyID *keyID = boost::get<CKeyID>(&destination);
		bool fInvalid = false;
		std::vector<unsigned char> vchSig = DecodeBase64(block.nVerify.c_str(), &fInvalid);
		CPubKey pubkey;
		...
		pubkey.RecoverCompact(ss.GetHash(), vchSig);
		if (pubkey.GetID() != *keyID) {
			LogPrintf("ERROR : Miner Key Validation Failed.\n");
			return false;
		}
	}
