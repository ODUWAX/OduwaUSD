OduwaUSD Coin version 0.12.0 is now available from:

  <https://oduwausd_coin.org/bin/oduwausd_coin-core-0.12.0/>
...
installer (on Windows) or just copy over /Applications/OduwaUSD_Coin-Qt (on Mac) or
oduwausd_coind/oduwausd_coin-qt (on Linux).
...
backwards-compatible with pre-0.10 versions of OduwaUSD Coin or other software:
...
pre-0.12 versions of OduwaUSD Coin or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start OduwaUSD Coin version 0.11 or
earlier.
...
ECDSA signatures inside OduwaUSD_Coin transactions now use validation using
[libsecp256k1](https://github.com/oduwausd_coin-core/secp256k1) instead of OpenSSL.
...
(https://github.com/oduwausd_coin/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of OduwaUSD Coin had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of OduwaUSD Coin was to raise the default minimum
relay fee.

OduwaUSD Coin 0.12 will have a strict maximum size on the mempool. The
...
OduwaUSD Coin 0.12 also introduces new default policy limits on the length and
...
OduwaUSD Coin 0.12 nodes. OduwaUSD Coin will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/oduwausd_coin/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in OduwaUSD Coin 0.12 does not yet have support for
...
This allows running oduwausd_coind without having to do any manual configuration.
...
OduwaUSD Coin has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. OduwaUSD Coin will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In OduwaUSD Coin 0.12, when mempool limit has been reached a higher minimum
...
entirely in OduwaUSD Coin 0.13, and it is at this time undecided whether the
...
OduwaUSD Coin has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
OduwaUSD Coin automatically creates a hidden service to listen on, without
manual configuration. OduwaUSD Coin will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if OduwaUSD Coin is listening, and
...
OduwaUSD_Coind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals OduwaUSD_Coin
Core to use floating fees. By default, OduwaUSD Coin will use floating
fees.
...
(default: `0.0002` OWDT/kB).

At all times, OduwaUSD Coin will cap fees at `-maxtxfee=<x>` (default:
0.10) OWDT.
Furthermore, OduwaUSD Coin will never create transactions paying less than
...
`oduwausd_coin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/oduwausd_coin/oduwausd_coin/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/oduwausd_coin/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/oduwausd_coin/bips/blob/master/bip-0065.mediawiki)
...
Then, to tunnel a SSL connection on 22519 to a RPC server bound on localhost on port 12519 do:

    stunnel -d 22519 -r 127.0.0.1:12519 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /oduwausd_coinrpc>
        ProxyPass http://127.0.0.1:2519/
        ProxyPassReverse http://127.0.0.1:2519/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass oduwausd_coind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< oduwausd_coinrpc:<password>
    </Location>
...
Restarting oduwausd_coind will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of OduwaUSD_Coin-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new oduwausd_coin-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include oduwausd_coin-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix oduwausd_coin-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of oduwausd_coin-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override ODUWAUSD_COIND and ODUWAUSD_COINCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [oduwausd_coin-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add ODUWAUSD_COIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library liboduwausd_coinconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "OWDT" (Ross Nicoll)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/oduwausd_coin/).
