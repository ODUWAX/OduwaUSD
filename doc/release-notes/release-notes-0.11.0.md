OduwaUSD Coin version 0.11.0 is now available from:

  <https://oduwausd_coin.org/bin/oduwausd_coin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer OWDT/kB fee to be
...
For example, add the following to `oduwausd_coin.conf`:
...
related to the blockchain in the oduwausd_coin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows OduwaUSD Coin to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that OduwaUSD Coin will 
...
For an example script see [oduwausd_coin-submittx](https://github.com/laanwj/oduwausd_coin-submittx).
...
- #5334 `cf87536` liboduwausd_coinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new oduwausd_coin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only OduwaUSD_CoinAmountField
- #5753 `0f44672` Add oduwausd_coin logo to about screen
...
- #5809 `46bfbe7` Add oduwausd_coin-cli man page
...
- #5849 `41259ca` contrib/init/oduwausd_coind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename OduwaUSD_Coin-Qt.app to OduwaUSD_Coin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for oduwausd_coin-tx)
