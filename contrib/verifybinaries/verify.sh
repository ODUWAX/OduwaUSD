...
###   oduwausd_coincore.org and oduwausd_coin.org and compares them.
...
WORKINGDIR="/tmp/oduwausd_coin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://oduwausd_coincore.org"
HOST2="https://oduwausd_coin.org"
BASEDIR="/bin/"
VERSIONPREFIX="oduwausd_coin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'oduwausd_coin-',
...
if [ $? -ne 0 ]; then
   echo "oduwausd_coin.org failed to provide signature file, but oduwausd_coincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "oduwausd_coin.org and oduwausd_coincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the OduwaUSD Coin binary release signing key installed?"
   fi
